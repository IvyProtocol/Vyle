...
fn_wallcache() {
    ...
    [[ ! -e "${dcolDir}/auto/${h_sum}.dcol" ]] && "${scrRun}" "${w_sum}" "${dcolDir}/auto"
    [[ ! -e "${dcolDir}/dark/${h_sum}.dcol" ]] && "${scrRun}" -d "${w_sum}" "${dcolDir}/dark"
    [[ ! -e "${dcolDir}/light/${h_sum}.dcol" ]] && "${scrRun}" -l "${w_sum}" "${dcolDir}/light"
} >/dev/null 2>&1
...
fn_wallcache_thumb() {
    ...
    local sr_call="$(fl_wallpaper -t "${w_sum}" -f 1)"
    [[ ! -f "${colsDir}/${sr_call}.cols" ]] && magick "${w_sum}"[0] -strip -resize 1000 -gravity center -extent 1000 -quality 90 "${colsDir}/${sr_call}.cols"
    [[ ! -f "${thmbDir}/${sr_call}.thmb" ]] && magick "${w_sum}"[0] -strip -thumjbnail 500x500^ -gravity center -extent 500x500 "${thmbDir}/${sr_call}.thmb"
    [[ ! -f "${quadDir}/${sr_call}.quad" ]] && magick "${thmbDir}/${sr_call}.thmb" \( -size 500x500 xc:white -fill "rgba(0,0,0,7)" -draw "polygon 400,500 500,500 500,0 450,0" -fill black -draw "polygon 500,500 500,0 450,500" \) -alpha Off -compose CopyOpacity -composite png:"${quadDir}/${sr_call}.quad"
} >/dev/null 2>&1
...
fn_wallcache_force() {
    ...
    "${scrRun}" -d "${w_sum}" "${dcolDir}/dark"
    "${scrRun}" -l "${w_sum}" "${dcolDir}/light"
    "${scrRun}" "${w_sum}" "${dcolDir}/auto"
} >/dev/null 2>&1

export -f fn_wallcache fn_wallcache_force fn_wallcache_blur fn_wallcache_thumb fl_wallpaper hashmap
